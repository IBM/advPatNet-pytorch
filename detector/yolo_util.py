...
def wrap_detection_results(outputs, scale_size, input_size, skip=True):
    # print ([item.shape for item in outputs])
    new_outputs = [None for _ in outputs]
    for k in range(len(outputs)):
        if outputs[k] is not None:
            new_outputs[k] = resize_boxes(outputs[k], scale_size, input_size)
            if skip:
...
def resize_boxes(detection, scale_size, input_size):
    w, h = input_size
    rh, rw = float(h)/scale_size, float(w)/scale_size
    detection[:,0] *= rw
    detection[:,1] *= rh
    detection[:,2] *= rw
    detection[:,3] *= rh
