...
from trainer.advPatch_trainer import *
...
#    logger.info("Random seed: {}".format(args.seed))
    ...
    try:  # for unexpected error logging
        ...
        # create trainer
        adv_trainer = AdvPatchTrainer(adv_model, optimizer, scheduler, use_gpu=cuda, use_ohem=config['use_ohem'], ohem_ratio=config['ohem_ratio'])
       ...
       #                            config['val_grid_path'], config['val_tps_path'], config['use_loc_net'])
        ...
        if args.evaluate:  # evaluation
            ...
            val_overall_loss, val_det_loss, val_tv_loss, val_det_accuracy, val_speed, speed_data_loader = \
                adv_trainer.val(val_loader, config, result_dir=result_dir)
        ...
        if os.path.isfile(config['resume']):
            ...
            adv_model, epoch, iteration, _, best_det_loss = build_advPatch_model_from_checkpoint_file(model_module, checkpoint_model_path)
            print('Resume, previous detection loss %.4f at epoch %d' % (best_det_loss, epoch))
            start_epoch = epoch + 1
            start_iteration = iteration
            adv_trainer.optimizer.load_state_dict(state_dict['optimizer'])
            adv_trainer.scheduler.load_state_dict(state_dict['scheduler'])
        ...
        for epoch in range(start_epoch, config['epochs']):
            ...
            train_overall_loss, train_det_loss, train_tv_loss, train_speed, speed_data_loader, num_batches =\
                adv_trainer.train(train_loader, config, epoch, cur_iteration=start_iteration)
            train_log_msg = 'Train: [{:05d}/{:05d}]\tlr: {:.6f}\toverall_Loss: {:4.4f}\tdet_Loss: {:4.4f}\ttv_Loss: {:4.4f}\tSpeed: {:.2f} ms/batch\tData loading: {:.2f} ms/batch'.format(
                epoch + 1, config['epochs'], lr_,  train_overall_loss, train_det_loss, train_tv_loss, train_speed * 1000.0, speed_data_loader * 1000.0)
            if args.rank == 0:
            ...
            if epoch % 1 == 0:
                val_overall_loss, val_det_loss, val_tv_loss, val_det_accuracy, val_speed, speed_data_loader = \
                    adv_trainer.val(val_loader, config)
            ...
            if args.rank == 0:  # save once only!!!
                adv_trainer.save_advPatch_checkpoint(
                ...
                if is_best_so_far:
                    adv_trainer.save_advPatch_best_model(config, checkpoint_path)
