...
class Generator(nn.Module):
    #def __init__(self, config, use_cuda=True, device_ids=[0,1,2,3,4,5]):
...
#        x_stage2, offset_flow = self.fine_generator(x, x_stage1, mask)
        x_stage2 = self.fine_generator(x)
        return x_stage2
    ...
    '''
    Make sure that the input ranges between [0, 1]
    '''
    def forward(self, x):
        return self.forward_template(x)

    def generate(self, src_img, frame_img):
        return self.forward_template(src_img)

    '''
    The input 'x' is between [0, 1], so we need to renormalize it to [-1, 1]. The output is converted back to [0, 1].
    '''
    def forward_template(self, x):
        output = self.fine_generator(x * 2.0 - 1.0)
...
class FineGenerator(nn.Module):
    def __init__(self, input_dim, cnum, use_cuda=True, device_ids=None, output_dim=None):
...
#        self.allconv11 = gen_conv(cnum*8, cnum*4, 3, 1, 1)
    ...
    def forward(self, xin: torch.Tensor, add_input_back: bool = True): #x_stage1, mask):
...
#        xnow = torch.cat([x1_inpaint, ones, mask], dim=1)
        ...
        if not add_input_back:
            return x

#        x_stage2 = torch.clamp(x, -1., 1.)
        x = 2 * torch.tanh(x) # why multiplied by 2?
        x += xin
